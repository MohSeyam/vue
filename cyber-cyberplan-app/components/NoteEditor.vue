<template>
  <div class="p-4 bg-white dark:bg-gray-800 rounded shadow max-w-lg mx-auto">
    <input v-model="title" placeholder="Title" aria-label="عنوان الملاحظة" class="w-full mb-2 p-2 rounded border" />
    <textarea v-model="content" placeholder="Write your note in Markdown..." aria-label="محتوى الملاحظة" rows="8" class="w-full mb-2 p-2 rounded border font-mono"></textarea>
    <input v-model="tagsInput" placeholder="Tags (comma separated)" aria-label="الكلمات المفتاحية" class="w-full mb-2 p-2 rounded border" />
