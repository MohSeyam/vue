<template>
  <div class="flex flex-col items-center bg-white dark:bg-gray-800 rounded-xl shadow p-4 w-full">
    ...
    <div class="text-lg font-bold mb-1">{{ plan.currentDay?.day?.en || '' }}</div>
    <div class="text-xs text-gray-400 mb-1">{{ plan.getDayTopic(plan.currentDay) }}</div>
    <ul v-if="plan.currentDay?.tasks?.length" class="text-xs text-gray-600 dark:text-gray-300">
      <li v-for="t in plan.currentDay.tasks" :key="t.id">• {{ t.title?.en || t.id }}</li>
    </ul>
