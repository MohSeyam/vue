import { useTranslation } from "react-i18next";
import { useCyberPlan } from "../../hooks/useCyberPlan";
import { useEffect, useState } from "react";
import { getPlanData } from "../../services/dataService";

export default function AchievementsSummary() {
  const { t } = useTranslation();
  const { plan, notes, loading } = useCyberPlan();
  const [learningHours, setLearningHours] = useState(0);

  useEffect(() => {
    async function calcLearningHours() {
      const planData = await getPlanData();
      // جميع المهام المنجزة (id موجود في plan.tasks.done)
      const doneMap = {};
      plan.forEach(w => {
        (w.tasks || []).forEach(t => {
          if (t.id && t.done) doneMap[t.id] = true;
        });
      });
      const allTasks = planData.flatMap(week => (week.days || []).flatMap(day => day.tasks || []));
      const doneTasks = allTasks.filter(task => doneMap[task.id]);
      // duration بالدقائق، نحوله لساعات ونقرب لأقرب عدد صحيح
      const totalMinutes = doneTasks.reduce((sum, t) => sum + (t.duration || 0), 0);
      setLearningHours(Math.round(totalMinutes / 60));
    }
    calcLearningHours();
  }, [plan]);

  if (loading) return <div className="text-center text-slate-400">{t("loading", "جاري التحميل...")}</div>;

  return (
    <div className="flex flex-col gap-2 items-center justify-center w-full">
      <div className="flex gap-6">
        <div className="text-center">
          <div className="text-2xl font-extrabold text-emerald-600 dark:text-emerald-400">{learningHours}</div>
          <div className="text-xs text-slate-500 dark:text-slate-400 font-medium">{t("learningHours", "إجمالي ساعات التعلم")}</div>
        </div>
        <div className="text-center">
          <div className="text-2xl font-extrabold text-violet-600 dark:text-violet-400">{notes.length}</div>
          <div className="text-xs text-slate-500 dark:text-slate-400 font-medium">{t("notesCount", "عدد الملاحظات المكتوبة")}</div>
        </div>
      </div>
    </div>
  );
}
